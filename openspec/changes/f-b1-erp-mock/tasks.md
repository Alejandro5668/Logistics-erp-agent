...
- [x] 1.1 Create `app/__init__.py` as an empty module initialization file
- [x] 1.2 Create `app/tools/__init__.py` that exports `get_erp_data` for agent binding
- [x] 1.3 Create `requirements.txt` with `sqlalchemy>=2.0`, `langchain-core>=0.1`, `pytest>=7.0`
- [x] 1.4 Create `.gitignore` with entries for `data/erp_mock.db`, `__pycache__/`, `.pytest_cache/`, `*.pyc`

## Phase 2: ORM Model & Engine Layer

- [x] 2.1 Create `app/tools/erp_data.py` with SQLAlchemy 2.0 declarative base and `ErpOrder` model
...
- [x] 2.2 Add lazy-initialized `_get_engine()` function reading `ERP_DB_PATH` env var (default: `data/erp_mock.db`)
- [x] 2.3 Add `_get_session()` sessionmaker using the lazy engine
- [x] 2.4 Implement `_fetch_order(order_id: str) → ErpOrder | None` using ORM `select().where()` with bound parameters (verify no SQL string interpolation)

## Phase 3: Seed Module & Dataset

- [x] 3.1 Create `app/tools/erp_seed.py` with `SEED_ORDERS` constant (10 records minimum)
...
- [x] 3.2 Implement `seed_database(engine)` using `session.merge()` over `SEED_ORDERS` for idempotent upsert
- [x] 3.3 Add `__main__` entry point to `app/tools/erp_seed.py` for manual seeding: `python -m app.tools.erp_seed`
- [x] 3.4 In `erp_data.py`, call `create_all()` and `seed_database()` on first engine initialization (verify idempotency)

## Phase 4: LangChain Tool Adapter

- [x] 4.1 In `app/tools/erp_data.py`, add `@tool def get_erp_data(order_id: str) → dict` with LLM-ready docstring
  - Docstring must explain the lookup, list args, and mention the tool's use case (comparing invoices vs. ERP)
- [x] 4.2 Validate `order_id` (trim whitespace, return not-found if empty)
- [x] 4.3 Call `_fetch_order(order_id)` and map the result to a flat dict:
...
- [x] 4.4 Verify the tool's `.name`, `.description`, and `.args_schema` are present for LLM function calling

## Phase 5: Test Infrastructure

- [x] 5.1 Create `tests/__init__.py` as an empty module initialization file
- [x] 5.2 Create `tests/conftest.py` with:
...
- [x] 5.3 Update `app/tools/erp_data.py` to respect `ERP_DB_PATH` env var for test isolation

## Phase 6: Test Verification

- [x] 6.1 Create `tests/test_erp_data.py` with:
...
- [x] 6.2 Add **Integration: Seed Idempotency Test** — Run `seed_database()` twice, assert row count is unchanged and data matches single run
- [x] 6.3 Add **Integration: Tool Bindability Test** — Assert `get_erp_data.name`, `.args_schema` exist and `.invoke({"order_id": "ORD-1001"})` returns a dict without error
- [x] 6.4 Run `pytest -v tests/test_erp_data.py` and verify all tests pass (add to CI later if needed)
