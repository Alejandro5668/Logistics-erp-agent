...
- [x] 1.1 Add `chromadb>=0.5,<0.6` to `requirements.txt`
...
- [x] 1.2 Create `tests/embedding_stub.py` with `StubEmbeddingFunction` class
...
- [x] 2.1 Create `app/rag/corpus.py` with regulatory snippets and idempotent ingestion
...
- [x] 2.2 Create `app/rag/store.py` with lazy client, collection, EF injection, and @tool adapter
...
- [x] 2.3 Create `app/rag/__init__.py` to re-export `search_regulations`
...
- [x] 3.1 Modify `tests/conftest.py` to add `regulations_index` fixture
...
- [x] 4.1 Create `tests/test_rag_search.py` with comprehensive test suite (all spec scenarios)
  - Spec link: Tests cover all spec requirements (R1–R5) and design testing strategy
  
  **Correct-year hit (spec R2 scenario 1):**
  - [x] 4.1.1 Test `search_regulations("tolerancia en discrepancias fiscales", year=2024)` returns `REG-2024-001` with correct 1% text
  - Verify exact `year == 2024`, title matches, text is readable
  
  **Cross-year exclusion (spec R2 scenario 2, design: load-bearing test):**
  - [x] 4.1.2 Test same query `year=2022` returns only `REG-2022-001` with 5% text
  - Verify all returned docs have `year == 2022`
  
  - [x] 4.1.3 Test same query `year=2023` returns only `REG-2023-001` with 2% text
  - Verify all returned docs have `year == 2023`
  
  **Filter is not decorative (design: test approach):**
  - [x] 4.1.4 Test raw collection query (without `where` filter) returns top-3 spanning multiple years
  ...
  **Empty-result behavior (spec R3):**
  - [x] 4.1.5 Test `search_regulations(query, year=1999)` returns `[]`, no exception
  - Verify type is list, length is 0
  
  - [x] 4.1.6 Test `search_regulations("", year=2024)` returns list (may be empty), no exception
  - Empty query must not raise
  
  **Result shape (spec R5):**
  - [x] 4.1.7 Test result shape: verify each dict has exactly keys `{"doc_id", "title", "year", "source", "text", "score"}`
  ...
  **Ingestion idempotency (spec R4, design testing strategy):**
  - [x] 4.1.8 Test run `corpus.ingest_corpus(collection)` twice, assert `collection.count() == len(REGULATION_SNIPPETS)`
  - Verify no duplicate `doc_id` entries
  
  **Tool bindability (spec R1 scenario 2, design testing strategy):**
  - [x] 4.1.9 Test `search_regulations.name` exists (string), `.args_schema` exists
...
- [x] 4.2 Run full test suite and verify coverage
...
- [x] 5.1 Verify requirements.txt pin does not introduce conflicts
...
- [x] 5.2 Final integration check: Run full test suite
