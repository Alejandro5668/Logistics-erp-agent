...
### Requirement: Framework-Agnostic Core with a Thin LangChain Adapter

The detection/authorization core (`inspect_input`, `inspect_output`, the role and injection
catalogs) MUST remain importable and testable with zero `langchain` dependency. Only the adapter
module MAY import `langchain`, and it MUST expose the guardrail as a single middleware object
(both `before_model` and `after_model` hooks bound together) so a caller cannot wire in one hook
without the other.

#### Scenario: Core has no LangChain import

- GIVEN the role/injection/guardrail modules
- WHEN their source is inspected
- THEN none of them import `langchain` or `langchain_core`

#### Scenario: One middleware object carries both hooks

- GIVEN `build_security_middleware()` is called
- WHEN the returned object is inspected
- THEN it exposes both `before_model` and `after_model` bound to the same instance
